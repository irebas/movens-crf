...
# from funcs import calc_price, calc_price2, calc_superkomp_price, second_smallest, open_excel_file
...
#     df['price_zone_1a'] = np.where(df['price_zone_1a'].fillna(0) == 0, df['final_price'], df['price_zone_1a'])
...
#         final_role = df.loc[r, 'final_role']
#         l1 = df.loc[r, 'L1_name']
#         price_zone_1 = df.loc[r, 'price_zone_1a'] if not pd.isna(df.loc[r, 'price_zone_1a']) else 0
...
#     df6 = SQLite(DB_NAME).run_sql_query('SELECT * FROM input_final_grid')
#     df6 = pd.merge(left=df6[['product_id_plxb', 'brand_id', 'final_grid']],
...
#     df = pd.merge(left=df, right=df6[['product_id_plxb'] + [f'{col}_plxb' for col in cols]], how='left',
#                   left_on='product_id', right_on='product_id_plxb')
#     for col in cols:
